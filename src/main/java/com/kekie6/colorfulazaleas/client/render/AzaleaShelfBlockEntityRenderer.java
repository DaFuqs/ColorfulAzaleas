...
public class AzaleaShelfBlockEntityRenderer implements BlockEntityRenderer<AzaleaShelfBlockEntity, ShelfBlockEntityRenderState> {
    private static final float ITEM_SCALE = 0.25F;
    private static final float BOTTOM_ALIGNED_OFFSET = -0.25F;
    private final ItemModelManager itemModelManager;

    public AzaleaShelfBlockEntityRenderer(BlockEntityRendererFactory.Context context) {
    ...
    public void updateRenderState(
            AzaleaShelfBlockEntity azaleaShelfBlockEntity,
            ShelfBlockEntityRenderState shelfBlockEntityRenderState,
            float f,
            Vec3d vec3d,
            @Nullable ModelCommandRenderer.CrumblingOverlayCommand crumblingOverlayCommand
    ) {
        BlockEntityRenderer.super.updateRenderState(azaleaShelfBlockEntity, shelfBlockEntityRenderState, f, vec3d, crumblingOverlayCommand);
        shelfBlockEntityRenderState.alignItemsToBottom = azaleaShelfBlockEntity.shouldAlignItemsToBottom();
        DefaultedList<ItemStack> defaultedList = azaleaShelfBlockEntity.getHeldStacks();
        int i = HashCommon.long2int(azaleaShelfBlockEntity.getPos().asLong());

        for (int j = 0; j < defaultedList.size(); j++) {
            ItemStack itemStack = defaultedList.get(j);
            if (!itemStack.isEmpty()) {
                ItemRenderState itemRenderState = new ItemRenderState();
                this.itemModelManager.clearAndUpdate(itemRenderState, itemStack, ItemDisplayContext.ON_SHELF, azaleaShelfBlockEntity.getEntityWorld(), azaleaShelfBlockEntity, i + j);
