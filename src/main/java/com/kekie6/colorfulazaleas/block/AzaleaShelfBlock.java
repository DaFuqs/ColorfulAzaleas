...
public class AzaleaShelfBlock extends BlockWithEntity implements InteractibleSlotContainer, SideChaining, Waterloggable {
    ...
    public static final MapCodec<AzaleaShelfBlock> CODEC = createCodec(AzaleaShelfBlock::new);
    ...
    @Override
    public MapCodec<AzaleaShelfBlock> getCodec() {
        return CODEC;
    }
        ...
        if (world.getBlockEntity(pos) instanceof AzaleaShelfBlockEntity azaleaShelfBlockEntity && !hand.equals(Hand.OFF_HAND)) {
            ...
            if (optionalInt.isEmpty()) {
                ...
                if (!(Boolean)state.get(POWERED)) {
                    boolean bl = swapSingleStack(stack, player, azaleaShelfBlockEntity, optionalInt.getAsInt(), playerInventory);
                    if (bl) {
    ...
    private static boolean swapSingleStack(ItemStack stack, PlayerEntity player, AzaleaShelfBlockEntity shelf, int hitSlot, PlayerInventory playerInventory) {
        ItemStack itemStack = shelf.swapStackNoMarkDirty(hitSlot, stack);
        ...
        shelf.markDirty(GameEvent.ITEM_INTERACT_FINISH);
